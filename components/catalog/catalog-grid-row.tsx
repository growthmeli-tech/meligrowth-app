...
export const CATALOG_GRID_ROW_CLASS =
  "grid w-full grid-cols-[minmax(0,1fr)_88px_minmax(112px,1fr)] gap-0 text-left text-sm align-top [@container(min-width:1024px)]:grid-cols-[32px_48px_minmax(200px,2fr)_72px_96px_72px_88px_120px_minmax(120px,1fr)_40px]";
...
function CatalogGridRowInner({
  ...
  return (
    ...
    >
      <div role="cell" className="hidden items-start p-2 [@container(min-width:1024px)]:flex">
        <input type="checkbox" checked={selected} onChange={onToggleSelect} aria-label={`Seleccionar ${row.item_id}`} />
      </div>
      <div role="cell" className="hidden p-2 [@container(min-width:1024px)]:block">
        {row.thumbnail ? (
      ...
      <div role="cell" className="hidden p-2 text-xs font-medium text-[#1A1A1A] [@container(min-width:1024px)]:block" title="Envío ML (publicación, datos ML)">
        {row.mlOfficial.publicationLogisticsLabel}
      </div>
      <div role="cell" className="hidden p-2 tabular-nums [@container(min-width:1024px)]:block">
