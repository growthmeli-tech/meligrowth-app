...
import { BarChart2, Bell, CheckSquare, LayoutDashboard } from "lucide-react";
...
const NAV_ITEMS: Array<{
  href: "/ops/dashboard" | "/ops/alerts" | "/ops/tasks" | "/ops/blocks";
  label: string;
  icon: LucideIcon;
          ...
          return (
            <li key={item.href}>
              <Link
                href={item.href}
                className={cn(
                  "relative flex flex-col items-center gap-1 px-2 py-2 text-[11px] font-semibold",
                  active ? "text-[#1A1A1A]" : "text-[#6B6B6B]"
                )}
              >
                <Icon className={cn("h-4 w-4", active ? "text-[#1A1A1A]" : "text-[#6B6B6B]")} />
                <span>{item.label}</span>
                {badgeValue > 0 ? <span className="absolute right-4 top-1 rounded-full bg-red-600 px-1.5 text-[10px] font-bold text-white">{badgeValue}</span> : null}
              </Link>
            </li>
          );
        })}
