...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Temporary safe redirect to avoid 404 while validating account-level routing/permissions.
    const finalRedirectUrl = new URL(
      `/internal/dashboard?ml_connected=true&company_id=${mlAccount.company_id}&ml_account_id=${state}`,
      request.url
    );
